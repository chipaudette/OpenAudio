...
//TympanRevC                tympanHardware;  //or change it to TympanRevC
//TympanRevD                tympanHardware;  //or change it to TympanRevC
TympanPins                tympPins(TYMPAN_REV_D0); //TYMPAN_REV_C or TYMPAN_REV_D
...
void setup() {
  ...
  //AudioMemory(10); 
  AudioMemory_F32(10,audio_settings); //allocate both kinds of memory
  ...
  Serial.print("Setup complete for Tympan Rev Number: ");
  Serial.println(audioHardware.getTympanRev());
} //end setup()
...
void servicePotentiometer(unsigned long curTime_millis, unsigned long updatePeriod_millis) {
  ...
  if ((curTime_millis - lastUpdate_millis) > updatePeriod_millis) { //is it time to update the user interface?
    ...
    //read potentiometer
    float val = float(audioHardware.readPotentiometer()) / 1023.0; //calls analogRead() [which returns 0 to 1023] and this scales 0.0 to 1.0
