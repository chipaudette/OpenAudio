...
void setup() {
  ...
  SerialUSB.print("Beginning FIR Trial (float): N_FIR_MAX = ");
  SerialUSB.println(N_FIR_MAX);
}
...
void loop() {
  ...
  for (int I_loop = 0; I_loop < 5; I_loop++) {
    ...
    SerialUSB.print("N_FIR = ");
    SerialUSB.print(N_FIR);
    SerialUSB.print(", After ");
    SerialUSB.print(N_TRIALS);
    SerialUSB.print(" Trials, One Trial Averages ");
    SerialUSB.print(micros_per_trial);
