...
const float sample_rate_Hz = 16000.0f ; //24000 or 44117.64706f (or other frequencies in the table in AudioOutputI2S_F32
...
int makeAudioConnections(void) { //call this in setup() or somewhere like that
  ...
  for (int i = 0; i < N_CHAN; i++) {
    //audio connections
    #if 1
        patchCord[count++] = new AudioConnection_F32(feedbackCancel, 0, bpFilt[i], 0); //connect to FIR filter
    #else
