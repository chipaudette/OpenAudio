...
*   Purpose: Do active noise cancelation.  Feedback topology.
*
*   Uses Tympan Audio Adapter.
*   Blue potentiometer adjusts the digital gain applied to the audio signal.
*   
*   Assumes microphones in ears and headphones over ears outside of the
*   microphones.
*   
...
void setup() {
  ...
  Serial.println("ANC_Feedback: starting setup()...");
...
void servicePotentiometer(unsigned long curTime_millis, unsigned long updatePeriod_millis) {
  ...
  if ((curTime_millis - lastUpdate_millis) > updatePeriod_millis) { //is it time to update the user interface?
    ...
    if (abs(val - prev_val) > 0.05) { //is it different than before?
      ...
      float vol_knob_gain = -(pow10f(vol_knob_gain_dB/20.f));

      //command the new gain setting
      gain1.setGain(vol_knob_gain);  //set the gain of the Left-channel gain processor
      gain2.setGain(vol_knob_gain);  //set the gain of the Right-channel gain processor
      Serial.print("servicePotentiometer: (Inverted) Digital Gain dB = "); Serial.println(vol_knob_gain_dB); //print text to Serial port for debugging
    }
